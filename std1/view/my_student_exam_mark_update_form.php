            		...
            		<div class="panel panel-primary">
        				<div class="panel-heading">               
        					<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>
          					<h3 class="panel-title">Edit Exam / Assessment Marks</h3>
   						</div>
...
while($row1=mysqli_fetch_assoc($result1)){
	...
	$sql2="select subject.id as s_id,subject.name as s_name,student_exam.marks as std_emark,student_exam.id as std_emark_id
       	   from student_subject
	       inner join subject_routing
	       on student_subject.sr_id=subject_routing.id
		   inner join subject
	       on subject_routing.subject_id=subject.id
		   inner join student_exam
	       on student_exam.index_number=student_subject.index_number
	       where student_subject.year='$current_year' and student_subject.sr_id='$sr_id' and student_subject.index_number='$std_index' and student_exam.exam_id='$exam_id' and student_exam.subject_id='$subject_id'";
...
?> 		
		<tr id="tr_<?php echo $count; ?>">
            ...
            <input type="hidden" name="id" value="<?php echo $row2['std_emark_id']; ?>"/>
        	<td id="eSubject_td_<?php echo $count; ?>"><?php echo $row2['s_name']; ?></td>
            <td id="eMark_td_<?php echo $count; ?>"><input type="text" name="exam_mark[]" value="<?php echo $row2['std_emark']; ?>" id="exam_mark_<?php echo $count; ?>"></td>
        </tr>
<?php	}} ?>
                                        </tbody>
                                    </table>
